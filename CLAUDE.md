...
**Brand config** lives entirely in `lib/brand.ts` (name, tagline, currency, timezone, socials, map links) and `app/globals.css` (CSS variables for the color palette). The Tailwind theme in `tailwind.config.ts` maps those CSS variables. Change brand details in one place — nowhere else.

**Contact info (phone / email / address / whatsapp) is DB-sourced, NOT static.** The `phone`/`email`/`address`/`whatsapp` fields in `lib/brand.ts` are **placeholders** (`+94 77 XXX XXXX`, `hello@…`) and must never be shown to customers. The live, admin-editable values live in the `shop_info` setting — read them via `getShopInfo()` (`lib/settings.ts`, server-only, returns a typed `ShopInfo`). Every customer-facing surface (storefront info pages, the bulk-request contact card, `MaintenanceGate`, the `/quote/[token]` page, and all email footers in `lib/notifications/*` + `lib/marketing/actions.ts`) sources contact details this way, and the storefront layout passes `shopInfo` down to the `Footer`, floating `WhatsAppButton`, and JSON-LD. When adding any UI or email that shows contact details, call `getShopInfo()` — do not reach for `brand.phone`/`brand.email`/`brand.address`. (`brand.name` is fine anywhere; it's stable identity, not contact info.)
...
**Subscription schedule semantics** (`lib/account/actions.ts`): the reminder rhythm is anchored to the order's delivery date (`next_reminder_date = delivery_date + interval` at creation, advanced by one interval per send). **Changing frequency preserves that anchor** — the new date is `(current next_reminder_date − old interval) + new interval`, rolled forward past today so an edit never fires a reminder immediately. **Pause → resume deliberately re-anchors to today + interval** (so a long-paused subscription doesn't fire instantly). Don't "simplify" either to `today + interval` — that detaches the schedule from the delivery rhythm (a fixed bug).
...
**Two daily crons** (`CRON_SECRET`-guarded, accept GET+POST, scheduled in `vercel.json`): `app/api/cron/subscription-reminders` (fires due reminders, advances `next_reminder_date`, never charges) feeding the one-tap `/reorder?sub=<id>` page (`reorderSubscription` in `lib/account/reorder.ts` → `components/storefront/ReorderClient.tsx`), and `app/api/cron/review-requests` (orders delivered exactly 2 days ago, deduped via `notification_logs`). The reminder cron **skips subscriptions whose product is unavailable** (unpublished / soft-deleted / out of tracked stock — mirroring the account page's availability rule) *without* advancing `next_reminder_date`, so reminders auto-resume on republish/restock. Known limitation: it sends **one email per due subscription row**, so a customer with several due subscriptions gets several emails (per-user grouping is a planned fix). A third route, `app/api/cron/cleanup-storage`, runs the orphan-image sweep with the 7-day buffer but **no-ops unless the `storage_cleanup` setting is enabled** (default OFF) — see "Storage cleanup" under Admin Panel. It is **not scheduled in `vercel.json`** (Vercel's free plan allows only two cron jobs, used by the daily pair) — trigger it manually, or re-add a weekly entry on a paid plan. The minute-level dispatcher stays on pg_cron (post-deploy). The newsletter + contact forms post to `lib/marketing/actions.ts`. There's a dev helper `npm run test-welcome you@email.com "Name"` to preview the welcome email (uses `scripts/tsconfig.email.json` so React-Email JSX renders correctly under tsx).
